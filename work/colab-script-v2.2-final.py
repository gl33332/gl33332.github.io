...
def get_file_signature(filepath):
    ...
    return f"{os.path.basename(filepath)}|{size}"
...
def split_and_send_telegram(text, filename):
    """【长文本分发】强制纯文本模式，避免特殊字符导致 Telegram 拒收报错"""
    ...
    for idx, chunk in enumerate(chunks):
        ...
        try:
            # 明确 parse_mode 为空，发送最原始的安全纯文本
            data = {"chat_id": TELEGRAM_CHAT_ID, "text": payload, "parse_mode": ""}
    ...
    for attempt in range(max_retries):
        try:
            ...
            cmd = [
                ...
                "--output_dir", TRANSCRIPT_DIR  # 强制输出到专属目录
            ]
            ...
            if result.returncode == 0 and os.path.exists(txt_file_path):
                ...
                print(f"⚠️ 引擎返回异常: {result.stderr[-200:]}")  # 只打印最后200个错误字符避免刷屏
            ...
            for file in all_files:
                ...
                if file_sig and file_sig not in processed_signatures:
                    ...
                    if content:
                        ...
                        # 只有成功推送，才把特征码写入免疫名单
                        save_to_log(HISTORY_LOG, final_sig)
