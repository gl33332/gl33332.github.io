---
title: AI 免费部署指南：利用 Cloudflare Workers 零成本搭建文生图平台
...
![AI免费部署文生图](https://imgbed.138gl.com/file/blog/文章/AI/1755614107881_TWAIbt-001.png)
...
2.  **登录平台**：访问并登录 [Cloudflare 控制台](https://dash.cloudflare.com/)。
...
<details>
<summary>📄 点击展开 worker.js 完整代码</summary>
    ```javascript
/**
 * @author: kared
...
5.  **设置访问密码**：
    * 在 `worker.js` 中找到以下位置（约第 75 行）：
    ```javascript
    // 💡 提示：在下面这行修改你的访问密码
    const PASSWORDS = ['admin123']; 
    ```
    * **关键步骤：** 将 `'admin123'` 替换为你自定义的密码，防止他人滥用你的算力额度。（注意保留单引号）。

6.  **添加前端界面 (`index.html`)**：
    * 在 Workers 编辑器左侧文件栏，点击 “+” 或右键点击 **“New File”**。
    * 文件名：务必填入 `index.html`（全小写）。
    * [🔗 点击此处跳转 GitHub 复制 ](https://github.com/gl33332/138gl-ai/blob/main/index.html)，或者展开下方代码块复制：

<details>
<summary>📄 点击展开 index.html 完整代码</summary>
    ```html
<!DOCTYPE html>
...
### 第三阶段：资源绑定（关键）
8.  **进入设置**：返回 Workers 项目主页，点击 **设置 (Settings)** -> **变量 (Variables)**。
9.  **添加 AI 绑定**：
    * 在下方找到 **“Workers AI 绑定”** 部分，点击添加绑定。
    * 名称通常保持默认，保存即可。

### 第四阶段：域名接入
10. **添加自定义域**：在项目页点击 **设置** -> **触发器 (Triggers)** -> **添加自定义域**。
11. **填写域名**：输入您的域名（例如 `ai.138gl.com`），点击添加。
12. **解析生效**：等待 Cloudflare 完成 DNS 解析后，即可通过该域名直接访问并开始 AI 绘图！
